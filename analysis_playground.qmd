...
```{r}
df8 <- read.csv('data/addingFOMC/SPY_ready_for_analysis_triple_witch_NFP_CSI_FOMC.csv', header = TRUE)
df8$NFPOutcomeIsShock <- ifelse(df7$NFPOutcome == "Shock", 1, 0)
df8$CSIOutcomeIsShock <- ifelse(df7$CSIOutcome == "Shock", 1, 0)
...
summary(null_model)
full_model <- glm(WasMondayOpenHigherThanFridayClose ~ didFridayCloseHigherThanThursdayClose + FridayCloseComparedToThursdayCloseChangeInPercent + wasFridayIntradayPositive + FridayIntradayChangeInPercent + fridayWasATripleWitchDay + NFPDayOrNot + NFPOutcome + CSIOutcome + CSIDayOrNot + wasItFOMCWeek + FOMCAction + FOMCActualVSExpected + FOMCActionSpecial + NFPOutcomeIsShock + CSIOutcomeIsShock, 
                  data = df8, family = binomial)
summary(full_model)
